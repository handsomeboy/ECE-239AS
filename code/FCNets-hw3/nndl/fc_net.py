...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(1, self.num_layers):
      ...
      out, cache = affine_relu_forward(input, w, b) if i != self.num_layers - 1 else affine_forward(out, w, b)
    ...
    for i in range(self.num_layers-1, 0, -1):
      ...
      dx, dw, db = affine_relu_backward(incoming_grad, layer_caches[i-1]) if i != self.num_layers - 1 else affine_backward(incoming_grad, layer_caches[i-1])
