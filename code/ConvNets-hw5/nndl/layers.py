...
def affine_forward(x, w, b):
  ...
  prod = np.prod(x.shape[1:])
  out = x.reshape((x.shape[0], prod)).dot(w) + b
...
def affine_backward(dout, cache):
  ...
  dw = x.reshape((x.shape[0], prod)).T.dot(dout)
...
def relu_backward(dout, cache):
  ...
  # so if x was > 0 there, the derivative is 1 else 0
  dx = dout * (x > 0)
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    unscaled_out = (x - mean) / (np.sqrt(var+eps))
    out = gamma * unscaled_out + beta
    running_mean = momentum * running_mean + (1 - momentum) * mean
    running_var = momentum * running_var + (1 - momentum) * var
    # everything i might possibly need...
    cache = (x, gamma, beta, eps, mean, var, unscaled_out, out)
    ...
    out = gamma*out + beta
    # don't need cache for test, because we won't be backpropagating gradients during test time
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
...
def batchnorm_backward(dout, cache):
  ...
  # ================================================================ #
  x, gamma, beta, eps, running_mean, running_var, unscaled_out, out = cache
  dx_scaled = dout * gamma
  stdev = np.sqrt(running_var + eps)
  dmean = (-1/stdev) *np.sum(dx_scaled,axis=0)
  dlda = (1/stdev) * dx_scaled
  partial = np.sum((x-running_mean)*dx_scaled, axis=0)
  dvar = -1/2*((running_var+eps)**-1.5) * partial
  # add the first term
  dx = dlda + 2*(x - running_mean)/x.shape[0] * dvar
  # add the second term
  dx+= 1/x.shape[0] * dmean
  # scaling grads
  dgamma = np.sum(dout*unscaled_out, axis=0)
...
def dropout_backward(dout, cache):
  ...
  if mode == 'train':
    ...
    # ================================================================ #
    # just send the gradient through if the input (i.e. the dropped-out x) into this was > 0, else 0 the grad.
    dx = dout * mask
    # ================================================================ #
    # END YOUR CODE HERE
    # ================================================================ #
...
def softmax_loss(x, y):
  ...
  N = x.shape[0]
  loss = -np.sum(np.log(probs[np.arange(N), y].clip(min=np.finfo(float).eps))) / N
  dx = probs.copy()
  dx[np.arange(N), y] -= 1
  dx /= N
  return loss, dx
